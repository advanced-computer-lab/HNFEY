...
const EditFlightSummary = (props) => {
  ...
  useEffect(() => {
    findFlight(
      "http://localhost:8000/hnfey/flight/" + reservation.departingFlightId
    ).then((res) => {
      setDepartingFlight(res.data.flight);
    });

    findFlight(
      "http://localhost:8000/hnfey/flight/" + reservation.returnFlightId
    ).then((res) => {
      setReturnFlight(res.data.flight);
    });
  }, []);

  return reservation._id && departingFlight._id && returnFlight._id ? (
    <Container component="main" style={{ marginTop: "2%" }}>
      ...
      <Paper elevation={6} style={{ margin: "5% 0% 2%" }}>
          ...
          <Grid item md={6}>
            ...
            >
              ...
              >
                ...
                >
                  x{Number(reservation.passengers.length)} Passengers
                </Typography>
              </div>
              <div
                style={{
        ...
        {reservation.passengers.map((passenger, i) => (
          <div key={i}>
            <div
              style={{
                margin: "3% 5% 2%",
...
export default EditFlightSummary;
